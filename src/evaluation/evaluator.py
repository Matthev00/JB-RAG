...
class RAGEvaluator:
    ...
    def evaluate(retriever: FAISSRetriever, dataset: RAGDataset, **retriever_params):
        ...
        for question, expected_files in dataset:
            ...
            # Precision@10
            precision = len(retrieved_files & expected_files) / len(retrieved_files) if retrieved_files else 0
            precision_scores.append(precision)

            # Recall@10
            recall = len(retrieved_files & expected_files) / len(expected_files) if expected_files else 0
            ...
            # MRR
            rank = next((i + 1 for i, res in enumerate(results) if res["relative_path"] in expected_files), 0)
...
results = RAGEvaluator.evaluate(retriever, RAGDataset(Path("data/escrcpy.json")), radius=0.3)
print(results)